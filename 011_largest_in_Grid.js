function largestGridProduct(arr) {
  let hLength = arr.length;
  let vLength = arr[0].length;
  let maxProduct = 1;
  for(let i=0; i<vLength; i++){
    for(let j=0; j<hLength; j++){
      if(j+3 < hLength){
        // horizontal products
        let hproduct = arr[i][j]*arr[i][j+1]*arr[i][j+2]*arr[i][j+3];
        if(maxProduct < hproduct){
          maxProduct = hproduct;
        }
          // forward diagonal products
          if (i+3<vLength){
            let fdproduct = arr[i][j]*arr[i+1][j+1]*arr[i+2][j+2]*arr[i+3][j+3];
            // console.log(arr[i][j], arr[i+1][j+1], arr[i+2][j+2], arr[i+3][j+3], fdproduct);
            if(maxProduct < fdproduct){
              maxProduct = fdproduct;
            }
          }
      }
      // vertical products
      if(i+3<vLength){
        let vproduct = arr[i][j]*arr[i+1][j]*arr[i+2][j]*arr[i+3][j];
        if(maxProduct < vproduct){
          maxProduct = vproduct;
        }
        if(j>2){
          let bdroduct = arr[i][j]*arr[i+1][j-1]*arr[i+2][j-2]*arr[i+3][j-3];
          // console.log(i,j,i+1,j-1,i+2,j-2,i+3,j-3);
          if(maxProduct < bdroduct){
